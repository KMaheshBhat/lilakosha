...
def run(config: dict) -> None:
    """LilaKosha Refinement Pass: Genre & Theme Classification with robust parsing."""
    ...
    logger.info(f"Classifying Genre & Themes in: {os.path.basename(latest_ledger)}")
    with open(latest_ledger, "r", encoding="utf-8") as f_in:
        lines = f_in.readlines()
    updated_sessions = []
    service_url = f"{config['services']['inspector']}/v1/chat/completions"
    for i, line in enumerate(tqdm(lines, desc="Genre & Theme Classification")):
        ...
        try:
            ...
            print(
                f" > [{i + 1}] "
                f"Classified:\n"
                f"  - Genre: {extracted_data.primary_genre.value}\n"
                f"  - Themes: {', '.join(extracted_data.themes)}\n"
            )
            annotations = [
                Annotation(
    ...
    logger.info("✅ Genre & theme refinement complete.")
